...
double myresult = 0;
VMCHAR cmyresult[101] = "Invalid Expression !";
int x, y;
int myresult1 = 0;

void vm_main(void) { 
    vm_ascii_to_ucs2(editor_title, 100, "Input expresion:");
    vm_input_set_editor_title((VMWSTR)editor_title);
    vm_input_text2(0, 4, f1); 
}
...
void f1(VMINT state, VMWSTR text) {
    vm_ucs2_to_ascii(v, 150, text);
    myresult = parse_string(v); //result int
    if (myresult1 != 1) {sprintf(cmyresult, "%01.2f", myresult);}
    vm_ascii_to_ucs2((VMWSTR)vvv, 100, cmyresult);
    vm_ascii_to_ucs2(editor_title, 100, "Result:");
    vm_input_set_editor_title((VMWSTR)editor_title);
