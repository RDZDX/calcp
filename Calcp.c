...
void f1(VMINT state, VMWSTR text) {
    ...
    if (state == VM_INPUT_OK && lenght > 0) {
       ...
       vm_file_write(f_read, v, strlen(v), &nwrite);
       vm_file_write(f_read, "=", strlen("="), &nwrite);
       vm_file_write(f_read, cmyresult, strlen(cmyresult), &nwrite);
...
void create_app_txt_filename(VMWSTR text) {
    ...
    vm_ascii_to_ucs2(text, (strlen("result.txt") + 1) * 2, "result.txt");
}
